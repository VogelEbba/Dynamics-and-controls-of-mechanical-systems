...
D0 = double(subs(D_0, ...
    {dx, dW, d_theta}, ...
    {dx_val, dW_val, d_theta_val}));

K0 = double(subs(K_0, ...
    {kx, k_theta, Lx, theta_ref}, ...
    {kx_val, k_theta_val, Lx_val, 0}));
...
%% State-space matrix A for linearized system
A = [zeros(2) eye(2);
     -M0\Ktot   -M0\D0];
...
%% ode45 integration of linearized system
tspan = [0 110];
lin_ode = @(t,z) A*z;       
